...
### 6.5. Optional: Generate Spec Documentation (lean-spec Integration)
- Ask user if they want to document the completed work
- If yes: invoke **spec-summary-generator** agent
- Creates post-hoc documentation based on **actual work done**
- Supports:
  - **Full documentation**: complete SPEC.md with decisions, files, coverage
  - **Key decisions only**: minimal record of architectural choices
  - **Experiments**: subspec for abandoned approaches
...
## lean-spec Integration (dev-first approach)

Unlike spec-first workflows, /dev uses lean-spec for **post-hoc documentation**:

```
Traditional (spec-first):
  /spec → SPEC.md → /dev → dev-plan.md → execute
  (Two translations, high hallucination risk)

/dev approach (dev-first):
  /dev → dev-plan.md → execute → optional SPEC.md
  (Single source of truth, documents reality)
```
...
|----------|----------------|
| Complex feature with multiple decisions | ✅ Full documentation |
| Simple feature, straightforward impl | ⚠️ Key decisions only |
| Trivial change, obvious approach | ❌ Skip |
| Tried multiple approaches | ✅ Record experiments |

### Spec Document Content (post-hoc)

```markdown
## Overview
[From Step 1: what was built and why]

## Implementation Summary
[From dev-plan.md: actual task breakdown]

## Key Decisions
[From conversation: architectural choices made]

## Files Changed
[From Step 6: actual files modified]

## Test Coverage
[From Step 5: real coverage numbers]
...
After generating spec, link dependencies:
```bash
lean-spec link {feature} --depends-on {other-feature}
```

Benefits of post-hoc dependency analysis:
- Based on actual code dependencies (not guesses)
- Reflects real implementation relationships
- More accurate than upfront planning
...
└── agents/
    ...
    └── spec-summary-generator.md      # Post-hoc spec documentation agent
```
